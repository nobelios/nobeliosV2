...
if (isset($_SESSION['login']) && isset($_SESSION['password']) && isset($_GET['path'])) {
	...
	if (file_exists('./..' . $_GET['path']) && in_array('properties.txt', $pageFolder = testDir($pageDir = './..' . $_GET['path'] . '/..'))) {
		...
		$author			= preg_replace('#.*\[author\](.+)\[/author\].*#i', '$1', $getProperties); 	// Récupération de l'id de l'auteur (découpage du texte parsé)
		$title			= preg_replace('#.*\[title\](.+)\[/title\].*#i', '$1', $getProperties);		// Récupération du titre de la page (découpage du texte parsé)
		...
		if (($author == $userId) || ($level == 4)) {
			...
			$folderSize = folderSize($dir);		// Taille du dossier en octets
			...
			if (isset($_FILES['filesend'])) {
				...
				if ($_FILES['filesend']['error'] == 0){
					...
					$fileName 	= substr($pathParts['basename'], 0, strrpos($pathParts['basename'], '.'));	// Récupération du nom (découpage de la position de 0 au début de l'extension)
					...
					$alert .= (($folderSize + $_FILES['filesend']['size']) <= $fileFolderMax)				? '' : $message['alert_upload_err_disk_space'] . '<br />'; 	// Espace disque insuffisant
					$alert .= ($_FILES['filesend']['size'] <= $fileMaxSize)									? '' : $message['alert_upload_err_file_size'] . '<br />';	// Fichier trop volumineux
					$alert .= (in_array($fileExt, $allowedFileExt))											? '' : $message['alert_upload_err_file_ext'] . '<br />';	// Extension non autorisée
					...
					if ($alert == null) {
						...
						*/
						...
						// si le nom existe le fichier est écrasé, seuls 40 caractères sont copiés
						$newName = substr($fileName, 0, 40) . '.' . $fileExt;
						// Affichage d'un message les copie réussies
						...
						// Affichage d'un message pour les copies échouées
				...
				$alert .= '<div class="pageButton">
					<a href="./index.php?files&amp;path=' . $_GET['path'] . '">' . $message['action_close']  . '<br /></a>
				</div>'; 
			}
			...
			if (isset($_GET['remove']) && isset($_GET['id'])) {
				...
				if (isset($_GET['id']) && in_array($_GET['id'], testdir($dir))) {	
					...
					if (isset($_GET['remove']) && isset($_GET['confirm'])) {
						...
						' . $_GET['id'] . '<br />
						<div class="pageButton">
							<a href="./index.php?files&amp;path=' . $_GET['path'] . '&amp;id=' . $_GET['id'] . '&amp;remove&amp;confirm">' . $message ['action_confirm_yes'] . '</a>
							<a href="./index.php?files&amp;path=' . $_GET['path'] . '">' . $message['action_confirm_no'] . '</a>
						</div>';
					}
				}
			}
			...
			elseif (isset($_POST['remove'])) {
				...
				if (isset($_POST['confirm'])) {
					...
					if ($nbFilesRemove != 0) {
						$alert .= '<form action="./index.php?files&amp;path=' . $_GET['path'] . '" method="post">';
			...
			elseif (isset($_GET['id']) && in_array($_GET['id'], $files)) {
				...
				$fileBigIcon	= '<img class="iconBig" src="./../' . $userDesignPath . '/images/icon_big_' . $filesExtType[$fileExt] . '.png" alt=""';																														// Affichage d'un icone selon le type de fichier
				$fileInsert		= '<a href="#" onclick="opener.insertTag(\'&lt;link url=&quot;' . $_GET['path'] . '/' . $_GET['id'] . '&quot; title=&quot;&quot;&gt;Télécharger&lt;/link&gt;\',\'\',\'textarea\',\'files\')">' . $message['action_file_insert'] . '</a>';	// Insertion du fichier
				$fileDownload	= '<a href="./../' . $_GET['path'] . '/' . $_GET['id'] . '">' . $message['action_file_download'] . '</a>';																																	// Téléchargement du fichier
				$fileRemove		= '<a href="./index.php?files&amp;path=' . $_GET['path'] . '&amp;id=' . $_GET['id'] . '&amp;remove">' . $message['action_file_remove'] . '</a>';																							// Effacement du fichier
				$fileInfo		= true;
			}
			...
			$maxSizeAllowed 		= $fileMaxSize/1000 . ' Ko';															// Taille maximale par fichier
			$filesAvailable 		= nbFolder($dir);																		// Nombre de fichiers disponibles
			$folderSpaceUsed 		= round($folderSize/1000) . " Ko / ".round($fileFolderMax/1000);						// Espace utilisé / espace total
			$folderSpaceAvailable 	= round(($fileFolderMax-$folderSize)/1000) . " Ko / " . round($fileFolderMax/1000);		// Espace disponible / espace total
			...
			echo '<div class="pageFolderInfo">
				<div class="pageImage">
					<img src="./../' . $userDesignPath . '/images/files.png" alt="" />
				</div>
				<div class="pageText">
			...
			echo '<form method="post" action="./index.php?files&amp;path=' . $_GET['path'] . '" enctype="multipart/form-data">
				<p>
			...
			if ($_SESSION[$sessionPage . 'view'] == 'list') {
				echo '<div class="pageList">
					<form class="pageListForm" action="./index.php?files&amp;path=' . $_GET['path'] . '" method="post">
						<div style="text-align: left; padding-bottom: 10px; margin-top: 0px; padding-top: 0px;">
							' . $message['legend_view_type'] . '<a href="./index.php?files&amp;path=' . $_GET['path'] . '&amp;view=miniatures">' . $message['action_view_miniatures'] . '</a>
						</div>
						<div>
									...
									<td class="pageListLegendCell" style="width: 500px;">';
										if ($_SESSION[$sessionPage . 'order'] == 'name') {
											if ($_SESSION[$sessionPage . 'sort'] == 'desc')
												echo '<a href="./index.php?files&amp;path=' . $_GET['path'] . '&amp;order=name&amp;sort=asc">' . $message['legend_name'] . ' <img class="legendSortButton" src="./../' . $userDesignPath . '/images/down.png" title="' . $message['info_order_up'] . '" alt="" /></a>';
											else
												echo '<a href="./index.php?files&amp;path=' . $_GET['path'] . '&amp;order=name&amp;sort=desc">' . $message['legend_name'] . ' <img class="legendSortButton" src="./../' . $userDesignPath . '/images/up.png" title="' . $message['info_order_down'] . '" alt="" /></a>';
										} else {
											echo '<a href="./index.php?files&amp;path=' . $_GET['path'] . '&amp;order=name&amp;sort=asc">' . $message['legend_name'] . '</a>';
										}
									echo '</td>
									<td class="pageListLegendCell" style="width: 80px;">';
										if ($_SESSION[$sessionPage . 'order'] == 'size') {
											if ($_SESSION[$sessionPage . 'sort'] == 'desc')
												echo '<a href="./index.php?files&amp;path=' . $_GET['path'] . '&amp;order=size&amp;sort=asc">' . $message['legend_size'] . ' <img class="legendSortButton" src="./../' . $userDesignPath . '/images/down.png" title="' . $message['info_order_up'] . '" alt="" /></a>';
											else
												echo '<a href="./index.php?files&amp;path=' . $_GET['path'] . '&amp;order=size&amp;sort=desc">' . $message['legend_size'] . ' <img class="legendSortButton" src="./../' . $userDesignPath . '/images/up.png" title="' . $message['info_order_down'] . '" alt="" /></a>';
										} else {
											echo '<a href="./index.php?files&amp;path=' . $_GET['path'] . '&amp;order=size&amp;sort=asc">' . $message['legend_size'] . '</a>';
										}
									echo '</td>
									<td class="pageListLegendCell">';
										if ($_SESSION[$sessionPage . 'order'] == 'type') {
											if ($_SESSION[$sessionPage . 'sort'] == 'desc')
												echo '<a href="./index.php?files&amp;path=' . $_GET['path'] . '&amp;order=type&amp;sort=asc">' . $message['legend_type'] . ' <img class="legendSortButton" src="./../' . $userDesignPath . '/images/down.png" title="' . $message['info_order_up'] . '" alt="" /></a>';
											else
												echo '<a href="./index.php?files&amp;path=' . $_GET['path'] . '&amp;order=type&amp;sort=desc">' . $message['legend_type'] . ' <img class="legendSortButton" src="./../' . $userDesignPath . '/images/up.png" title="' . $message['info_order_down'] . '" alt="" /></a>';
										} else {
											echo '<a href="./index.php?files&amp;path=' . $_GET['path'] . '&amp;order=type&amp;sort=asc">' . $message['legend_type'] . '</a>';
										}
									echo '</td>
								</tr>
							</table>
							...
							foreach ($tableOrder as $key=>$val) {
								echo '<tr class="pageListRowColor1" onclick="checkTheBox(\'table1_chk\', ' . $i . ');">
									<td class="pageListCell" style="width: 500px" id="table1_chk' . $i . '">
										...
										<img class="editechButtonImg" src="./../' . $userDesignPath . '/editech/' . $language . '/insert.png" title="' . $message['info_insert'] . '" onclick="checkTheBox(\'table1_chk\', ' . $i . '); opener.insertTag(\'&lt;link url=&quot;' . $_GET['path'] . '/' . $tableName[$key] . '&quot; title=&quot;&quot;&gt;Télécharger&lt;/link&gt;\',\'\',\'textarea\',\'files\')" alt="" />
										<a href="./index.php?files&amp;path=' . $_GET['path'] . '&amp;id=' . $tableName[$key] . '" onclick="checkTheBox(\'table1_chk\', ' . $i . ');">' . cutString($tableName[$key], 53, 1) . '</a>
									</td>
			...
			elseif ($_SESSION[$sessionPage . 'view'] == 'miniatures') {
				echo '<div id="list" class="pageList">
					<form id="form" class="pageListForm" action="./index.php?files&amp;path=' . $_GET['path'] . '" method="post">
						<div style="text-align: left; padding-bottom: 10px">
							' . $message['legend_view_type'] . '<a href="./index.php?files&amp;path=' . $_GET['path'] . '&amp;view=list">' . $message['action_view_list'] . '</a>
						</div>
						<div>
							<table cellspacing="0px" class="pageListLegendTable">
								<tr class="pageListLegendRow">
						...
						if ($nbFiles == 0) {
							...
							foreach ($tableOrder as $key=>$val) {
								echo '<div class="pageListIconDivFrame">
									<a href="./index.php?files&amp;path=' . $_GET['path'] . '&amp;id=' . $tableName[$key] . '">
