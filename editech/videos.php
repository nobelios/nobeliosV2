		...
		if (($author == $userId) || ($level == 4)) {
			...
			if (isset($_FILES['filesend'])) {
				...
				if ($_FILES['filesend']['error'] == 0){
					...
					$alert .= (($folderSize + $_FILES['filesend']['size']) <= $videoFolderMax)				? '' : $message['alert_upload_err_disk_space'] . '<br />'; 	// Espace disque insuffisant
					$alert .= ($_FILES['filesend']['size'] <= $videoMaxSize)								? '' : $message['alert_upload_err_file_size'] . '<br />';	// Fichier trop volumineux
					$alert .= (in_array($fileExt, $allowedVideoExt))										? '' : $message['alert_upload_err_file_ext'] . '<br />';	// Extension non autorisée
				...
				$alert .= '<div class="pageButton">
					<a href="./index.php?videos&amp;path=' . $_GET['path'] . '">' . $message['action_close']  . '<br /></a>
				</div>'; 
			}
			...
			if (isset($_GET['remove']) && isset($_GET['id'])) {
				...
				if (isset($_GET['id']) && in_array($_GET['id'], testdir($dir))) {	
					...
					if (isset($_GET['remove']) && isset($_GET['confirm'])) {
						...
						' . $_GET['id'] . '<br />
						<div class="pageButton">
							<a href="./index.php?videos&amp;path=' . $_GET['path'] . '&amp;id=' . $_GET['id'] . '&amp;remove&amp;confirm">' . $message ['action_confirm_yes'] . '</a>
							<a href="./index.php?videos&amp;path=' . $_GET['path'] . '">' . $message['action_confirm_no'] . '</a>
						</div>';
					}
				}
			}
			...
			elseif (isset($_POST['remove'])) {
				...
				if (isset($_POST['confirm'])) {
					...
					if ($nbFilesRemove != 0) {
						$alert .= '<form action="./index.php?videos&amp;path=' . $_GET['path'] . '" method="post">';
			...
			elseif (isset($_GET['id']) && in_array($_GET['id'], $files)) {
				...
				$fileName 		= $message['legend_file_name'] . ' : ' . $_GET['id'];																																									// Nom du fichier
				...
				$flashType		= ($fileExt == "mp3") ? "sound" : "video";
				$fileInsert		= '<a href="#" onclick="opener.insertTag(\'&lt;' . $flashType . ' url=&quot;' . $_GET['path'] . '/' . $_GET['id'] . '&quot;&gt;\',\'\',\'textarea\',\'flash\')" alt="">' . $message['action_file_insert'] . '</a>';		// Insertion du fichier
				$fileDownload	= '<a href="./../' . $_GET['path'] . '/' . $_GET['id'] . '">' . $message['action_file_download'] . '</a>';																												// Téléchargement du fichier
				$fileRemove		= '<a href="./index.php?videos&amp;path=' . $_GET['path'] . '&amp;id='. $_GET['id'] . '&amp;remove">' . $message['action_file_remove'] . '</a>';																		// Effacement du fichier
				$fileInfo		= true;
				
				// Type de présentation (fichier son ou vidéo)
				if ($fileExt == "mp3")
					$fileBigIcon = '<object width="350px" height="20px" type="application/x-shockwave-flash" data="./../flash/player.swf"><param name="movie" value="./../flash/player.swf" /><param name="flashvars" value="file=' . $file . '" /></object>';	// Affichage d'un apperçu du fichier son
				else
					$fileBigIcon = '<object width="480px" height="360px" type="application/x-shockwave-flash" data="./../flash/player.swf"><param name="movie" value="./../flash/player.swf" /><param name="flashvars" value="file=' . $file . '" /></object>';	// Affichage d'un apperçu de la vidéo
			...
			$extAllowed = '';
			for ($i=0; $i<count($allowedVideoExt); $i++) {
				$extAllowed = $extAllowed.$allowedVideoExt[$i];
				$extAllowed = ($i == (count($allowedVideoExt) - 1)) ? $extAllowed : $extAllowed . ' / '; // Ajoute de "/" entre les extentions autorisées
			}
			
			$folderSize 			= folderSize($dir);																		// Mise à jour de la taille du dossier images		
			$maxSizeAllowed 		= $videoMaxSize/1000 . ' Ko';															// Taille maximale par fichier
			$filesAvailable 		= nbFolder($dir);																		// Nombre de fichiers disponibles
			$folderSpaceUsed 		= round($folderSize/1000) . " Ko / ".round($videoFolderMax/1000);						// Espace utilisé / espace total
			$folderSpaceAvailable 	= round(($videoFolderMax-$folderSize)/1000) . " Ko / " . round($videoFolderMax/1000);	// Espace disponible / espace total
			
			//-----------------------------------------------------------------
			// Partie Affichage
			//-----------------------------------------------------------------
			
			echo $message['message_editech_files_title'] . ' : ' . $title;
			
			// Affichage des informations du dossier
			echo '<div class="pageFolderInfo">
				<div class="pageImage">
					<img src="./../' . $userDesignPath . '/images/videos.png" alt="" />
				</div>
				<div class="pageText">
			...
			if ($fileInfo != null)
				echo '<div class="pageFileInfo"">
					...
					<div class="pageText">
						...
						' . $message['legend_file_link'] . ' : ' . $fileInsert . ' / ' . $fileDownload . ' / ' . $fileRemove . '<br />
						<div class="pageButton">
							<a href="./index.php?videos&amp;path=' . $_GET['path'] . '">' . $message['action_close']  . '<br /></a>
						</div>
					</div>
			...
			echo '<div>
				<form method="post" action="./index.php?videos&amp;path=' . $_GET['path'] . '" enctype="multipart/form-data">
					<p>
			...
			if ($_SESSION[$sessionPage . 'view'] == 'list') {
				echo '<div class="pageList">
					<form class="pageListForm" action="./index.php?videos&amp;path=' . $_GET['path'] . '" method="post">
						<div style="text-align: left; padding-bottom: 10px; margin-top: 0px; padding-top: 0px;">
							' . $message['legend_view_type'] . '<a href="./index.php?videos&amp;path=' . $_GET['path'] . '&amp;view=miniatures">' . $message['action_view_miniatures'] . '</a>
						</div>
						<div>
									...
									<td class="pageListLegendCell" style="width: 500px;">';
										if ($_SESSION[$sessionPage . 'order'] == 'name') {
											if ($_SESSION[$sessionPage . 'sort'] == 'desc')
												echo '<a href="./index.php?videos&amp;path=' . $_GET['path'] . '&amp;order=name&amp;sort=asc">' . $message['legend_name'] . ' <img class="legendSortButton" src="./../' . $userDesignPath . '/images/down.png" title="' . $message['info_order_up'] . '" alt="" /></a>';
											else
												echo '<a href="./index.php?videos&amp;path=' . $_GET['path'] . '&amp;order=name&amp;sort=desc">' . $message['legend_name'] . ' <img class="legendSortButton" src="./../' . $userDesignPath . '/images/up.png" title="' . $message['info_order_down'] . '" alt="" /></a>';
										} else {
											echo '<a href="./index.php?videos&amp;path=' . $_GET['path'] . '&amp;order=name&amp;sort=asc">' . $message['legend_name'] . '</a>';
										}
									echo '</td>
									<td class="pageListLegendCell" style="width: 80px;">';
										if ($_SESSION[$sessionPage . 'order'] == 'size') {
											if ($_SESSION[$sessionPage . 'sort'] == 'desc')
												echo '<a href="./index.php?videos&amp;path=' . $_GET['path'] . '&amp;order=size&amp;sort=asc">' . $message['legend_size'] . ' <img class="legendSortButton" src="./../' . $userDesignPath . '/images/down.png" title="' . $message['info_order_up'] . '" alt="" /></a>';
											else
												echo '<a href="./index.php?videos&amp;path=' . $_GET['path'] . '&amp;order=size&amp;sort=desc">' . $message['legend_size'] . ' <img class="legendSortButton" src="./../' . $userDesignPath . '/images/up.png" title="' . $message['info_order_down'] . '" alt="" /></a>';
										} else {
											echo '<a href="./index.php?videos&amp;path=' . $_GET['path'] . '&amp;order=size&amp;sort=asc">' . $message['legend_size'] . '</a>';
										}
									echo '</td>
									<td class="pageListLegendCell">';
										if ($_SESSION[$sessionPage . 'order'] == 'type') {
											if ($_SESSION[$sessionPage . 'sort'] == 'desc')
												echo '<a href="./index.php?videos&amp;path=' . $_GET['path'] . '&amp;order=type&amp;sort=asc">' . $message['legend_type'] . ' <img class="legendSortButton" src="./../' . $userDesignPath . '/images/down.png" title="' . $message['info_order_up'] . '" alt="" /></a>';
											else
												echo '<a href="./index.php?videos&amp;path=' . $_GET['path'] . '&amp;order=type&amp;sort=desc">' . $message['legend_type'] . ' <img class="legendSortButton" src="./../' . $userDesignPath . '/images/up.png" title="' . $message['info_order_down'] . '" alt="" /></a>';
										} else {
											echo '<a href="./index.php?videos&amp;path=' . $_GET['path'] . '&amp;order=type&amp;sort=asc">' . $message['legend_type'] . '</a>';
										}
									echo '</td>
								</tr>
							</table>
							...
							foreach ($tableOrder as $key=>$val) {
								...
								echo '<tr class="pageListRowColor1" onclick="checkTheBox(\'table1_chk\', ' . $i . ');">
									<td class="pageListCell" style="width: 500px" id="table1_chk' . $i . '">
										...
										<img class="editechButtonImg" src="./../' . $userDesignPath . '/editech/' . $language . '/insert.png" title="' . $message['info_insert'] . '" onclick="checkTheBox(\'table1_chk\', ' . $i . '); opener.insertTag(\'&lt;' . $flashType . ' url=&quot;' . $_GET['path'] . '/' . $tableName[$key] . '&quot;&gt;\',\'\',\'textarea\',\'flash\')" alt="" />
										<a href="./index.php?videos&amp;path=' . $_GET['path'] . '&amp;id=' . $tableName[$key] . '" onclick="checkTheBox(\'table1_chk\', ' . $i . ');">' . cutString($tableName[$key], 53, 1) . '</a>
									</td>
			...
			elseif ($_SESSION[$sessionPage . 'view'] == 'miniatures') {
				echo '<div id="list" class="pageList">
					<form id="form" class="pageListForm" action="./index.php?videos&amp;path=' . $_GET['path'] . '" method="post">
						<div style="text-align: left; padding-bottom: 10px">
							' . $message['legend_view_type'] . '<a href="./index.php?videos&amp;path=' . $_GET['path'] . '&amp;view=list">' . $message['action_view_list'] . '</a>
						</div>
						<div>
							<table cellspacing="0px" class="pageListLegendTable">
								<tr class="pageListLegendRow">
						...
						if ($nbFiles == 0) {
							...
							foreach ($tableOrder as $key=>$val) {
								...
								echo '<div class="pageListIconDivFrame">
									<a href="./index.php?videos&amp;path=' . $_GET['path'] . '&amp;id=' . $tableName[$key] . '">
										<div class="pageListIconDivIcon">
											<img class="pageListIconNormal" src="./../' . $userDesignPath . '/images/icon_normal_' . $filesExtType[$tableType[$key]] . '.png" alt="" />
										</div>
